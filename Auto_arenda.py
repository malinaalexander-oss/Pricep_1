...
# ================== НАСТРОЙКИ ==================
TOKEN = "8151509290:AAHSTotV96q22XF9RuRL_zSWvNMJlkTbBHA"
OWNER_ID = 3437915
DATA_FILE = "data.json"
...
if not data["categories"]:
    data["categories"] = {
        "для дачи": [
            {"id": 101, "name": "Х123ХХ178", "location": "Пр. Науки 25"},
            {"id": 102, "name": "А456АА178", "location": "Пр. Науки 25"},
            {"id": 103, "name": "М789ММ178", "location": "Пр. Науки 25"},
        ],
        "открытый": [
            {"id": 201, "name": "К111КК178", "location": "Пр. Науки 25"},
            {"id": 202, "name": "Р222РР178", "location": "Пр. Науки 25"},
            {"id": 203, "name": "Н333НН178", "location": "Пр. Науки 25"},
        ],
        "закрытый": [
            {"id": 301, "name": "С444СС178", "location": "Пр. Науки 25"},
            {"id": 302, "name": "Т555ТТ178", "location": "Пр. Науки 25"},
            {"id": 303, "name": "Е666ЕЕ178", "location": "Пр. Науки 25"},
        ],
        "эндуро": [
            {"id": 401, "name": "В777ВВ178", "location": "Уманский пер. 77"},
            {"id": 402, "name": "О888ОО178", "location": "Уманский пер. 77"},
            {"id": 403, "name": "У999УУ178", "location": "Уманский пер. 77"},
        ],
    }
    save_data(data)
...
def is_available(trailer_id, desired_start, desired_end):
...
async def choose_category(callback: types.CallbackQuery):
    ...
    for i in range(7):
        ...
        kb.row(types.InlineKeyboardButton(text=f"{date.day} {months[date.month-1]}", callback_data=f"start_{i}"))
...
async def choose_start(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("Выберите длительность аренды:", reply_markup=kb.as_markup())
...
async def choose_duration(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    text_trailers = "Выберите прицеп:\n\n"
    for t in available_trailers:
        text_trailers += f"{t['name']} – {t['location']}\n"
    ...
    await callback.message.edit_text(text_trailers, reply_markup=kb.as_markup())
...
async def confirm_booking(callback: types.CallbackQuery):
    ...
    trailer = next(t for cat in data["categories"].values() for t in cat if t["id"] == trailer_id)
    ...
    await callback.message.edit_text(
        ...
        f"Аренда: {start.strftime('%d.%m.%Y')} – {end.strftime('%d.%m.%Y')}\n\n"
        f"В день начала аренды нажмите 'Забрать прицеп'.",
        reply_markup=kb.as_markup()
    )

# ================== ЗАБРАТЬ/СДАТЬ ПРИЦЕП ==================
...
async def photo_take(message: types.Message):
    ...
    user_state.pop(uid)
    await message.answer("✅ Фото отправлено владельцу. Кнопка 'Сдать прицеп' появится в меню.", reply_markup=get_main_button(uid))
